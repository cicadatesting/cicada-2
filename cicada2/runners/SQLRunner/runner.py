...
def run_action(action_type: str, params: ActionParams) -> ActionResult:
    ...
    if result.returns_rows:
        ...
        rows = [
            {item[0]: item[1] for item in row.items()}
            for row in result
        ]
    else:
        rows = []
...
def contains_rows(expected_rows: List[dict], actual_rows: List[dict]) -> Tuple[bool, str]:
...
def equals_rows(expected_rows: List[dict], actual_rows: List[dict]) -> Tuple[bool, str]:
    ...
    for expected_row, actual_row in zip(expected_rows, actual_rows):
        row_matches, description = assert_dicts(expected_row, actual_row, all_required=True)
